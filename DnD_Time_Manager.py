...
def update_db(db):
    ...
    if not version_compare(db_version):
        ...
        pickler(camp_dir / f"{campaign}.pkl", new_db)
        return new_db
...
def move_files(target_path):
    ...
    try:
        copytree("campaigns", str(target_path / "campaigns"))
    except Exception as e:
        error(e)
    else:
        if "campaigns" in listdir(target_path):
            ...
            error(f"{target_path / 'campaigns'} was not moved successfully")


    try:
        copyfile("pref.pkl", str(target_path / "pref.pkl"))
    except Exception as e:
        error(e)
    else:
...
if not DEV_MODE:
    user_area = Path(getenv('LOCALAPPDATA')) / 'JP-Carr' / 'DnD_Time_Manager'
else:
    user_area = Path(getenv('LOCALAPPDATA')) / 'JP-Carr' / 'DnD_Time_Manager_DEV'

if not user_area.exists(): #creates directory for save data
    user_area.mkdir()
...
if "campaigns" not in listdir(user_area):
    (user_area / "campaigns").mkdir()

campaign=None
if len(listdir(user_area / "campaigns"))!=0:    #loads most recent possible campaign
    for i in pref["last campaign"]:
        if i in listdir(user_area / "campaigns") and (user_area / "campaigns" / i / f"{i}.pkl").exists():
            campaign=i
            break

if campaign==None:
    for file in listdir(user_area / "campaigns"):
        if (user_area / "campaigns" / file / f"{file}.pkl").exists():
            campaign=file

if len(listdir(user_area / "campaigns"))==0 or campaign==None:
    campaign=popup.create_campaign(str(user_area) ,first=True, theme=pref["theme"])
...
main_layout=[
        ...
        [sg.InputText(f"{db.hour}:00",size=(6,1), readonly=True,key="hour_display", tooltip="Time - 24 hour"),   sg.InputText(db.day, size=(3,1), readonly=True,key="day_display", tooltip="Day of the month"),   sg.InputText(db.tenday, size=(2,1), readonly=True,key="tenday_display", tooltip="Tenday", visible=pref["show_tenday"]),   sg.InputText(f"{db.month[0]}. {db.month[1]}", size=(30,1), readonly=True,key="month_display", tooltip="Month"),   sg.InputText(db.year, size=(5,1), readonly=True,key="year_display", tooltip="Year - DR")],
...
while True:
    ...
    if event == sg.WIN_CLOSED:    #breaks loop if window is closed
        ...
        if window["log_input"].Get() not in (""," ","  "):
            ...
            try:
                with open(camp_dir / f"{campaign}.txt", "a") as log:
                    log.write(f"{db.hour}:00 {db.day}/{db.month[0]}/{db.year} - {window['log_input'].Get()}\n")

                window["log_input"].Update("")

            except PermissionError as e:
                error(f"Unable to print to {campaign}.txt - PermissionError")

            except Exception as e:
                error(f"Unable to print to {campaign}.txt - {str(e)}")
        ...
        try:
            startfile(str(camp_dir / f"{campaign}.txt"))
...
# Menu Events -----------------------------------------------------------------
    ...
    # File --------------------------------------------------------------------
        ...
        window.disable()
        campaign=popup.create_campaign(str(user_area), first=False, theme=pref["theme"], par_centre=centre)
        window.enable()
        ...
        Tk().withdraw()
        try:
            path=askdirectory(initialdir=str(user_area / "campaigns"))
        ...
        if new_campaign!=None:
            try:
                (user_area / "campaigns" / new_campaign).mkdir()
                for file in listdir(camp_dir):
                    file_type=file.split(".")[-1]
                    rename(str(camp_dir / file), str(user_area / "campaigns" / new_campaign / f"{new_campaign}.{file_type}"))
            except FileExistsError as e:
                window.disable()
                popup.alert_box(text=f"Capaign \"{new_campaign}\" already exists", theme=pref["theme"], par_centre=centre)
                window.enable()
            except Exception as e:
                window.disable()
        ...
        if popup.choice_box(text=f"Are you sure you want to delete campaign \"{campaign}\"?", window_name="Delete Campaign", theme=pref["theme"], par_centre=centre):
            send2trash(str(camp_dir))
            ...
            if len(listdir(user_area / "campaigns"))==0 or campaign==None:
                window.disable()
                campaign=popup.create_campaign(str(user_area), first=True, theme=pref["theme"], par_centre=centre)
                window.enable()
        ...
        startfile(str(user_area / "campaigns"))
    ...
    # Tools--------------------------------------------------------------------
        ...
        window.enable()
        error(f"Raw time request - {db.hour} hours, {db.day_raw} days", sound=False)
